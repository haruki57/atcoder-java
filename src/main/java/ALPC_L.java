...
public class ALPC_L {
    ...
    // https://atcoder.jp/contests/practice2/submissions/53961615
    public static void run (final FastScanner scanner, final PrintWriter out) {
        int n = scanner.nextInt(), q = scanner.nextInt();
        SegNode[] data = new SegNode[n];
        for(int i = 0; i < n; i++){
            int val = scanner.nextInt();
            data[i] = new SegNode(val == 0? 1: 0, val == 1? 1: 0, 0);
        }

        BinaryOperator<SegNode> op = (a, b) -> {
            int zero = a.zero + b.zero;
            int one = a.one + b.one;
            long inv = (long) a.one * b.zero + a.inv + b.inv;
            return new SegNode(zero, one, inv);
        };
        final SegNode E = new SegNode(0,0,0);
        BiFunction<Boolean, SegNode, SegNode> mapping = (flag, node) -> {
            int zero = flag ? node.one : node.zero;
            int one = flag ? node.zero : node.one;
            long inv = flag ? (long) zero * one - node.inv : node.inv;
            return new SegNode(zero, one,  inv);
        };
        BinaryOperator<Boolean> composition = Boolean::logicalXor;

        LazySegTree<SegNode, Boolean> sgt = new LazySegTree<>(
                data,
                op,
                E,
                mapping,
        ...
        for(int i = 1; i <= q; i++){
            ...
            if(t == 1) sgt.apply(l, r, true);
            else out.println(sgt.prod(l, r).inv);
        }
    }
    ...
    static class SegNode{
        int zero, one;
        long inv;
        public SegNode(int zero, int one, long inv) {
            this.zero = zero;
            this.one = one;
            this.inv = inv;
        }
