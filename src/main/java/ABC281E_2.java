...
//            out.println(right.map);
    ...
    private static void pop(long x, MultiSet left, MultiSet right) {
        ...
        if(left.size < K) {
            var smallest = right.map.firstKey();
            right.erase(smallest);
            left.insert(smallest);
        }
    }

    static void push(long x, MultiSet left, MultiSet right) {
        left.insert(x);
        if(left.size > K) {
            var biggest = left.map.lastKey();
            left.erase(biggest);
