...
public class ABC354E {
    ...
    static void run (final FastScanner scanner, final PrintWriter out) {
        ...
        for (int i = 0; i < memo.length; i++) {
            ...
            //System.out.println(Integer.toBinaryString(i)+" "+memo[i]);
        }
    }

    private static boolean rec(int bit, int[][] cards) {
        if(memo[bit]!=null) {
            return memo[bit];
        }
        ...
        for (int i = 0; i < N; i++) {
            ...
            for (int j = i+1; j < N; j++) {
                ...
                if(cards[i][0]==cards[j][0]||cards[i][1]==cards[j][1]) {
                    ...
                    if (!rec(newBit, cards)) {
                        return memo[bit]=true;
                        //canWin = true;
                    }
                }
            }
        }
        return memo[bit] = false;
    }
